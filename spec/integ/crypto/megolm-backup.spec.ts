...
describe.each(Object.entries(CRYPTO_BACKENDS))("megolm-keys backup (%s)", (backend: string, initCrypto: InitCrypto) => {
    ...
    describe("Key backup check on UTD message", () => {
        ...
        it("Alice checks key backups when receiving a message she can't decrypt", async () => {
            ...
            // On the first decryption attempt, decryption fails. Despite being a historical event which was sent before
            // our device was created, we expect MEGOLM_UNKNOWN_INBOUND_SESSION_ID rather than HISTORICAL_EVENT,
            // because backup is configured.
            await awaitDecryption(event);
            expect(event.decryptionFailureReason).toEqual(DecryptionFailureCode.MEGOLM_UNKNOWN_INBOUND_SESSION_ID);
